...
| [09 · 跨扩展通用协议](09-cross-extension-protocols.md) | 硬依赖怎样通过 package manifest 与 direct service 组合；Todo、Request 怎样向独立 extension 保留 compatibility channel；EventBus 的同步边界是什么 | Direct service、request/response、state broadcast、UI adapter |
| [10 · 通用能力注册调研](10-capability-registry-research.md) | 通用扩展怎样被硬依赖插件、可选 consumer、自定义 SDK 宿主和 stock Pi 复用；怎样从每能力一套 channel 演进到 typed capability | 依赖分流、capability discovery、直接 service、核心 facade |
...
flowchart LR
    ...
    Start -->|开发扩展| D[04 扩展系统] --> P[09 跨扩展协议] --> R[10 能力注册] --> E[06 玩法] --> F[07 生产检查]
    Start -->|构建新 Agent 产品| B --> G[05 生态与衍生 Agent] --> E
```

- 第一次接触 Pi：按 `01 → 02 → 03 → 04` 阅读。
- 已经会写 extension：先看 `04 → 09 → 10 → 06 → 07`，再用 `02/03` 解释边界问题。
