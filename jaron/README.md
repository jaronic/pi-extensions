...
- 输入区使用包含四角、上下边和左右边的完整闭合边框；Plan/Goal 状态统一嵌入上边框。
- 输入以 `!` 开头时，上边框显示 `! shell mode`；以 `!!` 开头时显示 `!! shell mode · no context`，并保留 Pi 原生 shell 模式边框色。Shell 与 Plan/Goal 同时存在时会并列显示。
- 输入框下方单独显示：当前 model、思考等级、session 名、项目路径、git branch 和 context window 占用。
