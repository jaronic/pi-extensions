...
test("LspClient stops a stubborn server process tree before shutdown resolves", async (context) => {
  ...
  const heartbeatPath = join(root, "heartbeats.log");
  context.after(async () => await rm(root, { recursive: true, force: true }));
  const client = await LspClient.start(fakeServerConfig("stubborn-tree", {
    command: [process.execPath, fakeServer, "stubborn-tree", pidPath, heartbeatPath],
  }), root, 3_000, () => {});
  ...
  const settledHeartbeats = await readFile(heartbeatPath, "utf8");
  await delay(100);
  assert.equal(await readFile(heartbeatPath, "utf8"), settledHeartbeats, "stubborn process tree remained live after shutdown");
});
