...
- `src/logger.ts`：默认开启 `error` 级、由全局 `hashline.json` 的 `logEnabled`/`logLevel` 控制、首次写入才创建日志目录、写入 `getAgentDir()/logs/hashline.log` 并有界轮转、按 CAS 拒绝/意外错误分级、C1 中和、吞掉自身失败的排查日志。与 lsp 的同名文件逐字节相同，避免跨包生产导入。
